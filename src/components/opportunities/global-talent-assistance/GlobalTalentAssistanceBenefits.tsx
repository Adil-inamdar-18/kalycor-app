...
import { globalTalentAssistanceBenefits } from "@/data/opportunities/globalTalentAssistance";

export default function GlobalTalentAssistanceBenefits() {
  return (
        ...
        <div className="grid gap-12 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
          <div>
            <p className="mb-4 text-sm font-semibold uppercase tracking-[0.2em] text-[#e9c176]">
              {globalTalentAssistanceBenefits.kicker}
            </p>

            <h2 className="max-w-xl text-3xl font-semibold leading-tight md:text-4xl lg:text-5xl">
              {globalTalentAssistanceBenefits.heading}
            </h2>
          </div>

          <div>
            <p className="max-w-2xl text-base leading-8 text-white/70 md:text-lg">
              {globalTalentAssistanceBenefits.body}
            </p>

            <ul className="mt-8 space-y-5">
              {globalTalentAssistanceBenefits.points.map((point) => (
                <li
                  key={point}
