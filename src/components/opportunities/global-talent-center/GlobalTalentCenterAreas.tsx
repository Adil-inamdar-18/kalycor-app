...
import { globalTalentCenterAreas } from "@/data/opportunities/globalTalentCenter";

export default function GlobalTalentCenterAreas() {
  return (
    <section className="bg-muted/30 py-20 md:py-28">
      <Container>
        <div className="mb-12 max-w-3xl">
          <p className="mb-4 text-sm font-semibold uppercase tracking-[0.2em] text-primary">
            {globalTalentCenterAreas.kicker}
          </p>

          <h2 className="text-3xl font-semibold leading-tight text-heading md:text-4xl lg:text-5xl">
            {globalTalentCenterAreas.heading}
          </h2>
        </div>

        <div className="grid gap-6 md:grid-cols-2">
          {globalTalentCenterAreas.items.map((item) => (
            <article
              key={item.number}
