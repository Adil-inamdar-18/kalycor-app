...
import { technologyCapabilities } from "@/data/solutions/technologySolutions";
      ...
      <Container>
        <div className="mb-12 max-w-3xl">
          <p className="mb-4 text-sm font-semibold uppercase tracking-[0.2em] text-primary">
            {technologyCapabilities.kicker}
          </p>

          <h2 className="text-3xl font-semibold leading-tight text-foreground md:text-4xl lg:text-5xl">
            {technologyCapabilities.heading}
          </h2>
        </div>

        <div className="grid gap-6 md:grid-cols-2">
          {technologyCapabilities.items.map((item) => (
            <article
              key={item.number}
